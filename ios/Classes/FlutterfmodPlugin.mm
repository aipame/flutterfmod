#import "FlutterfmodPlugin.h"
#import "fmod.h"
#import <AVFoundation/AVFoundation.h>
#import <EMVoiceConvert.h>
#import "amrFileCodec.h"
#import "ConversionAudio.hpp"
#import <AVFoundation/AVAudioSession.h>

@interface FlutterfmodPlugin () <AVAudioRecorderDelegate, AVAudioPlayerDelegate>
{
    NSError *_error;
    NSString *recordPath;
...
@property (nonatomic, strong) NSString *lastPlayPath;
@property (nonatomic, strong) NSString *lastWavPath;
...
@implementation FlutterfmodPlugin

+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  FlutterMethodChannel* channel = [FlutterMethodChannel
      methodChannelWithName:@"flutterfmod"
            binaryMessenger:[registrar messenger]];
  FlutterfmodPlugin* instance = [[FlutterfmodPlugin alloc] init];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    else if ([@"conversion" isEqualToString:call.method]){
        
        int conversionType = [call.arguments[@"conversionType"] intValue];
        int random = arc4random()%10;

        NSString *path = call.arguments[@"path"];
        NSString *savePath =  call.arguments[@"savePath"];
        NSString *wavPath  = [self _convertAudioFile:path retPaht:NULL];
        NSString *conversionPath = [NSString stringWithFormat:@"%@%d.wav", [self.path stringByAppendingFormat:@"/%.0f", [[NSDate date] timeIntervalSince1970] * 1000], random];
        if (savePath == nil || (NSNull *)savePath == [NSNull null] || savePath.length == 0) {
            savePath =  [NSString stringWithFormat:@"%@%d.amr", [self.path stringByAppendingFormat:@"/%.0f", [[NSDate date] timeIntervalSince1970] * 1000], random];
        }
        
      int code = FmodSoundSaveSound(wavPath.UTF8String, conversionType, conversionPath.UTF8String);
        if (code == 0) {
            BOOL ret = [self _convertWAV:conversionPath toAMR:savePath];
            if (ret) {
                result(savePath);
            } else {
                result([NSError errorWithDomain:@"转换失败" code:100001 userInfo:NULL]);
            }
        } else {
            result([NSError errorWithDomain:@"转换失败" code:100001 userInfo:NULL]);
        }
...
- (BOOL)_convertWAV:(NSString *)aWavPath toAMR:(NSString *)aAmrPath
{
    ...
    if ([fileManager fileExistsAtPath:aAmrPath]) {
        ...
        [FlutterfmodPlugin _wavPath:aWavPath toAmrPath:aAmrPath];
        if ([fileManager fileExistsAtPath:aAmrPath]) {
            ret = YES;
        }
    }
...
- (void)_startPlayerWithPath:(NSString *)aPath
    ...
    do {
        ...
        aPath = [self _convertAudioFile:aPath retPaht:NULL];
        if ([aPath length] == 0) {
...
- (NSString *)_convertAudioFile:(NSString *)aPath retPaht:(NSString *)savePaht
{
    if ([[aPath pathExtension] isEqualToString:@"mp3"]) {
        return aPath;
    }
    
    NSString *retPath = ( savePaht != NULL && savePaht.length > 0)? savePaht: [[aPath stringByDeletingPathExtension] stringByAppendingPathExtension:@"wav"];
    do {
        NSFileManager *fileManager = [NSFileManager defaultManager];
        ...
        if ([FlutterfmodPlugin _isMP3File:retPath]) {
            retPath = aPath;
            break;
        }
        
        [FlutterfmodPlugin _amrToWav:aPath wavSavePath:retPath];
        if (![fileManager fileExistsAtPath:retPath]) {
            retPath = nil;
        }
